  ...
  return (
    <div className="min-h-screen flex bg-background text-foreground transition-colors duration-400 font-sans selection:bg-emerald-500/30">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-muted border-r border-border overflow-hidden select-none items-center justify-center p-12">
        ...
        <div className="relative z-10 max-w-lg text-center lg:text-left rtl:text-right">
          ...
          <h2 className="text-4xl font-extrabold tracking-tight mb-6 leading-tight text-foreground">
            {isRTL ? 'انضم اليوم لإدارة الموارد الزراعية بدقة متناهية' : 'Create Your Precision Farming Account'}
          </h2>

          <p className="text-base text-muted-foreground leading-relaxed mb-10 font-semibold">
            {isRTL 
              ? 'بوابة تسجيل الحسابات الموحدة للمهندسين والمشرفين والمحاسبين. سجل حسابك الآن لبدء عمليات التتبع والتشغيل الذكي.' 
              : 'Enterprise-grade portal for engineers, managers, accountants, and executives. Create your profile to streamline crop operations.'}
          </p>
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-between p-6 sm:p-12 relative overflow-hidden">
        ...
        <div className="w-full max-w-md mx-auto z-10 my-auto py-8">
          ...
          <div className="text-center lg:text-left rtl:text-right mb-8">
            ...
            <p className="text-sm font-semibold text-muted-foreground">
              {t('auth.register_subtitle', 'انضم لمساحة عمل النظام الزراعي')}
            </p>
          </div>

          {serverError && (
            <div className="mb-5 p-4 bg-destructive/10 border border-destructive/20 text-destructive text-sm font-bold rounded-2xl flex items-center gap-3">
          ...
          {success && (
            <div className="mb-5 p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-sm font-bold rounded-2xl flex items-center gap-3">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <input
                      ...
                      className={`w-full bg-card border ${errors.name ? 'border-destructive focus:ring-destructive/30' : 'border-border focus:border-emerald-500 focus:ring-emerald-500/20'} rounded-2xl pl-12 pr-4 rtl:pl-4 rtl:pr-12 py-3.5 text-sm font-semibold text-foreground placeholder-muted-foreground/60 outline-hidden focus:ring-4 transition-all duration-200`}
                    />
                  )}
                />
              </div>
            ...
            {/* Role Select Dropdown (Styled natively to match shadcn/ui Select) */}
            <div className="space-y-1.5">
              <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
                {t('auth.role', 'تحديد الدور')}
              </label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 rtl:left-auto rtl:right-0 flex items-center pl-4.5 rtl:pl-0 rtl:pr-4.5 text-muted-foreground pointer-events-none z-10">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <select
                      {...field}
                      className={`w-full bg-card border ${errors.role ? 'border-destructive focus:ring-destructive/30' : 'border-border focus:border-emerald-500 focus:ring-emerald-500/20'} rounded-2xl pl-12 pr-10 rtl:pl-10 rtl:pr-12 py-3.5 text-sm font-semibold text-foreground outline-hidden focus:ring-4 transition-all duration-200 appearance-none cursor-pointer`}
                    >
                      {roles.map((option) => (
                        <option key={option.value} value={option.value} className="bg-card text-foreground font-semibold">
                          {option.label}
                        </option>
                      ))}
                    </select>
                ...
                />
                <span className="absolute inset-y-0 right-0 rtl:right-auto rtl:left-0 flex items-center pr-4 rtl:pr-0 rtl:pl-4 text-muted-foreground pointer-events-none">
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  <span>{t('auth.register_btn', 'تسجيل الحساب')}</span>
          ...
          <p className="text-center text-sm font-bold text-muted-foreground mt-8">
            ...
            <Link to="/login" className="text-emerald-500 hover:text-emerald-600 transition-colors mx-1">
        ...
        {/* Footer info text */}
        <div className="w-full text-center text-xs text-muted-foreground/75 mt-8 select-none">
