...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        palm_structure = [
            {'name': 'المرحلة 1', 'enclosures': [str(i) for i in range(1, 8) if i != 6]}, # 1->7 exclude 6
            {'name': 'المرحلة 2', 'enclosures': [str(i) for i in range(6, 21) if i != 7]}, # 6->20 exclude 7
            {'name': 'المرحلة 3', 'enclosures': [str(i) for i in range(21, 33)]}, # 21->32
            ...
            {'name': 'المرحلة 7', 'enclosures': [str(i) for i in range(12, 16)]}, # 12->15
        ...
        for idx, stage_data in enumerate(palm_structure, start=1):
            stage, created = Stage.objects.get_or_create(
                crop=palm_crop, name=stage_data['name'],
                defaults={'order': idx}
            )
            if created:
            ...
            for enc_idx, enc_name in enumerate(stage_data['enclosures'], start=1):
                enc, enc_created = Enclosure.objects.get_or_create(
                    crop=palm_crop, stage=stage, name=enc_name,
                    defaults={'order': enc_idx}
                )
                if enc_created:
                    self.stdout.write(self.style.SUCCESS(f'    Created Enclosure: {enc_idx} for Stage {idx}'))
        ...
        for idx, region_name in enumerate(olive_regions, start=1):
            enc, enc_created = Enclosure.objects.get_or_create(
                crop=olive_crop, stage=None, name=region_name,
                defaults={'order': idx}
            )
            if enc_created:
                self.stdout.write(self.style.SUCCESS(f'  Created Olive Region (Enclosure): {idx}'))

        self.stdout.write(self.style.SUCCESS('Farm structure seeding completed successfully!'))
