...
def migrate_report_dropdown_options(apps, schema_editor):
    ...
    for option in ReportDropdownOption.objects.all():
        ...
        option_type = getattr(option, "field_type", None) or getattr(
        ...
        if option_type == "contractor":
            ...
            Operation.objects.get_or_create(
                company=option.company, name=option.name, defaults={"is_active": True}
            )
    ...
    for report in DailyTaskReport.objects.all():
        ...
        if report.variety_id:
            if report.variety_id not in variety_map:
                option = ReportDropdownOption.objects.filter(
                    pk=report.variety_id
                ).first()
        ...
        if updated:
            report.save(
                update_fields=["variety_new_id", "contractor_new_id", "unit_new_id"]
            )
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(
            migrate_report_dropdown_options, reverse_migrate_report_dropdown_options
        ),
