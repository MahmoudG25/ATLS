...
class FarmSettingsSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            ...
            "enable_stage",
            "enable_enclosure",
            "allow_stage_without_sector",
            "allow_enclosure_without_stage",
        ]
        ...
        if parent:
            ...
            if parent.type == LocationNode.TYPE_ENCLOSURE:
                raise serializers.ValidationError(
                    {"parent": "لا يمكن إضافة عناصر تابعة للحوشة."}
                )

            if parent.type == LocationNode.TYPE_STAGE and node_type not in [
                LocationNode.TYPE_STAGE,
                LocationNode.TYPE_ENCLOSURE,
            ]:
                raise serializers.ValidationError(
                    {"type": "المرحلة يمكن أن تحتوي على مراحل أو حوشات فقط."}
                )

            if parent.type == LocationNode.TYPE_SECTOR and node_type not in [
                LocationNode.TYPE_STAGE,
                LocationNode.TYPE_ENCLOSURE,
            ]:
                raise serializers.ValidationError(
                    {"type": "القطاع يمكن أن يحتوي على مراحل أو حوشات فقط."}
                )
            ...
            if parent.level >= 2:
                raise serializers.ValidationError(
                    {"parent": "تم الوصول للحد الأقصى لعمق الهيكل (3 مستويات)."}
                )
